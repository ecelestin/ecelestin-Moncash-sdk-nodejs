...
var invoke = exports.invoke = function invoke(http_method, path, data, http_options_param, cb) {
    ...
    if (configuration.default_options.mode !== configuration.default_options.live && process.env.MONCASH_DEBUG) {
    ...
    //     console.warn('WARNING: openssl version ' + process.versions.openssl + ' detected. Per PCI Security Council mandate (https://github.com/moncash/TLS-update), you MUST update to the latest security library.');
    // }
    ...
    req.on('response', function (res) {
        ...
        res.on('end', function () {
            ...
            try {
                //export MONCASH_DEBUG to development to get access to moncash-debug-id
                //for questions to merchant technical services.
                if (res.headers['moncash-debug-id'] !== undefined && process.env.MONCASH_DEBUG) {
                    console.log('moncash-debug-id: ' + res.headers['moncash-debug-id']);

                    if (configuration.default_options.mode !== configuration.default_options.live) {
                ...
                err = new Error('Invalid JSON Response Received. If the response received is empty, please check' +
                 'the httpStatusCode attribute of error message for 401 or 403. It is possible that the client credentials' +
                  'are invalid for the environment you are using, be it '+configuration.default_options.live+' or '+configuration.default_options.sandbox+'.');
                err.error = {
            ...
            if (!err && (res.statusCode < 200 || res.statusCode >= 300)) {
                ...
                // that MONCASH returns and information link
                err.response = response;
                if (process.env.MONCASH_DEBUG) {
