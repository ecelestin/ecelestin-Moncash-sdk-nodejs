...
var configure = exports.configure = function configure(options) {
    ...
    if (configuration.default_options.mode !== configuration.default_options.sandbox && configuration.default_options.mode !== configuration.default_options.live) {
        throw new Error('Mode must be "'+configuration.default_options.sandbox+'" or "'+configuration.default_options.live+'"');
    }
};

/**
 * Generate new access token by making a POST request to /oauth/token by
...
var generateToken = exports.generateToken = function generateToken(config, cb) {
    ...
    var payload = 'scope=read,write&grant_type=client_credentials';
    ...
    var http_options = {
        ...
        host: getDefaultApiEndpoint(config.mode) || config.host || configuration.default_options.host,
    ...
    client.invoke('POST', configuration.default_options.oauth_token_uri, payload, http_options, function (err, res) {
        var token = null;
        if (res) {
...
/**
 * Makes a Moncash REST API call. Reuses valid access tokens to reduce
...
var executeHttp = exports.executeHttp = function executeHttp(http_method, path, data, http_options, cb) {
    ...
    http_options.host = getDefaultApiEndpoint(http_options.mode) || http_options.host;
    ...
    if (http_options.client_metadata_id) {
        http_options.headers['Moncash-Client-Metadata-Id'] = http_options.client_metadata_id;
    }
    else if (http_options.correlation_id) {
        http_options.headers['Moncash-Client-Metadata-Id'] = http_options.correlation_id;
    }
...
var redirectUri = exports.redirectUri = function redirectUri(path){
    if(configuration.default_options.mode == configuration.default_options.sandbox)
        return configuration.default_options["sandbox_redirect_uri"]+path;
    else if(configuration.default_options.mode == configuration.default_options.live)
        return configuration.default_options["live_redirect_uri"]+path;
};

var getDefaultApiEndpoint = exports.getDefaultApiEndpoint = function getDefaultApiEndpoint(mode) {
    if(mode == configuration.default_options.sandbox)
        return configuration.default_options["rest_sandbox_endpoint"];
    
    else if(mode == configuration.default_options.live)
        return configuration.default_options["rest_live_endpoint"];
